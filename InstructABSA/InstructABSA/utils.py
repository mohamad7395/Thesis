...
class T5Generator:
    ...
    def tokenize_function_inputs(self, sample):
        ...
        return model_inputs
    def train_with_metrics(self,indomain_dataset ,tokenized_datasets,epoch_adress ,**kwargs):
            ...
            # Define trainer with the custom MetricsCallback
            trainer = Seq2SeqTrainer(
                ...
                callbacks=[MetricsCallback(self, indomain_dataset=indomain_dataset, tokenized_dataset=tokenized_datasets, output_file = epoch_adress,batch_size=16)]
            )
...
class MetricsCallback(TrainerCallback):
    def __init__(self, t5_exp,indomain_dataset, tokenized_dataset, output_file,batch_size=16):
        self.t5_exp = t5_exp
        self.tokenized_dataset = tokenized_dataset
        self.indomain_dataset = indomain_dataset
        self.batch_size = batch_size
        self.output_file = output_file
        self.metrics_df = pd.DataFrame(columns=['epoch', 'sample_set', 'precision', 'recall', 'f1'])

    def on_epoch_end(self, args, state, control, **kwargs):
        self.t5_exp.model.eval()
        # Loop through each set (train, validation, test) and calculate metrics
        for sample_set in [ 'validation', 'test']:
            if sample_set not in self.tokenized_dataset:
                continue  # Skip if dataset split not available

            # Get predictions and true labels
            pred_labels = self.t5_exp.get_labels(self.tokenized_dataset, sample_set=sample_set, batch_size=16)
            true_labels = [i.strip() for i in self.indomain_dataset[sample_set]['labels']]

            # Calculate metrics
            p, r, f1, _ = self.t5_exp.get_metrics(true_labels, pred_labels)
        ...
        self.t5_exp.model.train()
    def on_train_end(self, args, state, control, **kwargs):
        # Save metrics DataFrame to CSV at the end of training
