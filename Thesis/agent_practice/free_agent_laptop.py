...
def evaluate_sentence(text: str) -> str:
    ...
    """
    print("_"*200)
    print('Evaluating sentence tool')
    prompt = f"""
    ...
    Example Input:
    The machine is slow to boot up and occasionally crashes completely. Terms= ['boot up'], Polarity= ['negative']
    OK
    I also got the added bonus of a 30" HD Monitor, which really helps to extend my screen and keep my eyes fresh! Terms= ['30" HD Monitor', 'screen'], Polarity= ['positive', 'neutral']
    OK
    The internet capabilities are really slow and frustrating. Terms: ['internet capabilities'] Polarity: ['negative']
    OK
    The laptop frequently shut down unexpectedly during use, very annoying. Terms: ['shut down'] Polarity: ['negative']
    OK

    Bad Example:
    The charging process is very slow and often takes a long time. Terms= ['charge'], Polarity= ['negative']
    NOT_OK
    (charcing process is aspect term, not charge)
    The laptop's 3G network is average, but the sound quality with the headphones is great, unfortunately, the i7 processor is not as powerful as expected. Terms= ['3G network', 'headphones', 'i7'], Polarity= ['neutral', 'positive', 'negative']
    NOT_OK
    (headphones is not aspect term in this sentence, also i7 "processor" is execcive)
    Make sure your output **exactly** follows this format. Do not include explanations.
    Input:
    {text}
    """
...
evaluator_agent = create_react_agent(
    ...
    prompt=""""
            ...
            ***MUST USE EACH TOOL ONLY ONCE.***
            ***DO NOT SKIP STEPS OR GUESS.ONLY GET ASNWERS FROM TOOLS***
"""
...
def evaluator_node(state: SentenceState):
    ...
    eval_input_format = f"USE YOUR TOOLS TO EVALUATE THIS SENTENCE : {generate_sentence} Terms={gen_terms} Polarity={gen_polarity}"
